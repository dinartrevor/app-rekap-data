...
@section('title', 'Edit Pencatatan Kasus - '.config('app.name'))
@section('content')
<div class="pagetitle">
    <h1>Edit Pencatatan Kasus</h1>
    <nav>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="#">Home</a></li>
        <li class="breadcrumb-item active">Edit Pencatatan Kasus</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
      ...
      <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                      <h5 class="card-title">Form Edit Pencatatan Kasus</h5>
                    </div>
                ...
                <form action="{{route('legal_case.update', $legalCase->id)}}" method="POST" enctype="multipart/form-data" id="form-create">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-4">
                            <label for="case_number" class="form-label">Nomor Perkara <span class="text-red">*</span></label>
                            <input type="text" class="form-control  @error('case_number') is-invalid @enderror" name="case_number" id="case_number" autocomplete="off" placeholder="Nomor Perkara" value="{{ $legalCase->case_number }}">
                            @error('case_number')
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control  @error('clarification') is-invalid @enderror"  name="clarification" id="clarification" autocomplete="off" placeholder="Klarifikasi Perkara" value="{{ $legalCase->clarification }}">
                            @error('clarification')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control @error('trial_date') is-invalid @enderror" name="trial_date" id="trial_date" autocomplete="off" placeholder="Tanggal Sidang" value="{{ $legalCase->trial_date }}">
                            @error('trial_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered fs-6">
                                    ...
                                    <tbody id="tbody-penggugat">
                                        @foreach ($legalCase->plaintiff as $key => $plaintiff)
                                        <tr>
                                            <td class="counter_penggugat">{{ $key + 1 }}</td>
                                            <td><input type="text" class="form-control" name="name_penggugat[]" autocomplete="off" placeholder="Nama" value="{{ $plaintiff->name }}"></td>
                                            <td><input type="text" class="form-control" name="place_of_birth_penggugat[]" autocomplete="off" placeholder="Tempat Lahir" value="{{ $plaintiff->place_of_birth }}"></td>
                                            <td><input type="date" class="form-control" name="date_of_birth_penggugat[]"  value="{{ $plaintiff->date_of_birth }}"></td>
                                ...
                                <table class="table table-bordered fs-6">
                                    <thead>
                                        ...
                                        <tbody id="tbody-tergugat">
                                            @foreach ($legalCase->defendant as $key => $defendant)
                                                <tr>
                                                    <td class="counter_penggugat">{{ $key + 1 }}</td>
                                                    <td><input type="text" class="form-control" name="name_tergugat[]" autocomplete="off" placeholder="Nama" value="{{ $defendant->name }}"></td>
                                                    <td><input type="text" class="form-control" name="place_of_birth_tergugat[]" autocomplete="off" placeholder="Tempat Lahir" value="{{ $defendant->place_of_birth }}"></td>
                                                    <td><input type="date" class="form-control" name="date_of_birth_tergugat[]"  value="{{ $defendant->date_of_birth }}"></td>
                                                    <td><button class="btn btn-danger btn-sm btn-remove"><i class="bi bi-trash"></i> Hapus</button></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="mediator" id="mediator" class="form-control" data-selectjs="true" data-placeholder="Mediator">
                                ...
                                <option value="Mediator 1" @selected($legalCase->mediator == 'Mediator 1')>Mediator 1</option>
                                <option value="Mediator 2" @selected($legalCase->mediator == 'Mediator 2')>Mediator 2</option>
                                <option value="Mediator 3" @selected($legalCase->mediator == 'Mediator 3')>Mediator 3</option>
                            </select>
                            @error('mediator')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-12 my-4">
                            ...
                            <textarea name="notes" id="notes" cols="30" rows="5" class="form-control @error('notes') is-invalid @enderror" placeholder="Keterangan Kasus">{{ $legalCase->notes }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-12">
                            ...
                            <textarea name="description" id="description" cols="30" rows="5" class="form-control" placeholder="Deskripsi">{{ $legalCase->description }}</textarea>
                        </div>
